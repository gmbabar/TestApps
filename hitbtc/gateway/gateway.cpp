...
std::string pubKey = R"(0e4x1WxctyiFxbvbcGVXuzawNfO2Ruy6)";
std::string priKey = R"(DjHTMhvOLe8QiKaGJ4EXGYQbzfXLqKcC)";
...
std::cout << "Listener:" << __func__ << std::endl;
        ...
        formattersSpotGateway::formatLogin(oss, priKey, pubKey);
        text_ = oss.str();
        ws_.async_write(
            net::buffer(text_),
            beast::bind_front_handler(
...
std::cout << "Listener:" << __func__ << std::endl;
        ...
        std::size_t bytes_transferred)
    {
        ...
        if(msg_count_ == 1) {
            auto res = jsonTree.get_optional<bool>("result");
            if(res) {
                std::ostringstream oss;
                formattersSpotGateway::formatSubscribe(oss, msg_count_);
                text_ = oss.str();
                this->socketSend();
            }
...
int main(int argc, char** argv)
{
    ...
    //     return EXIT_FAILURE;
    // }
    auto const host = R"(api.demo.hitbtc.com)";
    auto const port = R"(443)";
    auto const target = R"(/api/3/ws/trading)";
    // auto const text = R"({"method": "spot_subscribe","params": {},"id": 123})";
