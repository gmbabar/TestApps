...
int main(int argc, char *argv[]) {
	...
	const int queue_size = 80, msg_count = 10000;
	char buffer[256];
 	int datalen = 0;
 	...
 	for (int idx = 0; idx < msg_count; ++idx) {
                ...
                datalen = sprintf(buffer, "%ld Sender Time.", static_cast<long int>(t1.time_since_epoch().count()/divide) );
 		publisher.SendMessage(buffer, datalen+1, idx);
		//usleep(10);
 	}
	printf("Done sending msgs: %d\n", msg_count);
 	return 0;
}
