...
std::string getHeader(std::string &apiSecret, std::string &apiKey, std::string &b64Payload) {
    unsigned int len = SHA256_DIGEST_LENGTH;
	unsigned char hash[SHA256_DIGEST_LENGTH];
#if OPENSSL_VERSION_NUMBER >= 0x1010100fL
    HMAC_CTX * hmac = HMAC_CTX_new();
    HMAC_Init_ex(hmac, reinterpret_cast<const unsigned char*>(apiKey), apiKey.length(), EVP_sha256(), NULL);
    HMAC_Update(hmac, reinterpret_cast<const unsigned char*>(b64Payload.c_str()), b64Payload.length());
    HMAC_Final(hmac, hash, &len);
    HMAC_CTX_free(hmac);
#else
    HMAC_CTX hmac;
    ...
    oss << "{"
        ...
        << R"(,'Content-Length':"0")"
        << R"(,'X-GEMINI-APIKEY':")" << apiKey << '"'
        << R"(,'X-GEMINI-PAYLOAD':")" << b64Payload << '"'
        << R"(,'X-GEMINI-SIGNATURE':")" << signature << '"'
...
inline std::string fmtGeminiSpotRestApiOrder(
        std::string &symbol, std::string &amount,
        std::string &price, std::string &side,
        std::string &type, std::string (&options)[S]) {

    std::ostringstream oss;
    oss << "{"
        << R"("request":"/v1/order/new")"
        << R"(,"nonce":)" << geminiTimestamp()
        << R"(,"symbol":")" << symbol << '"'
        << R"(,"amount":")" << amount << '"'
        << R"(,"price":")" << price << '"'
        << R"(,"side":")" << side << '"'
...
inline bool fmtGeminiSpotRestApiCancel(
    ...
    signGeminiRestRequest(aRequest);
    return true;
}

inline bool fmtGeminiSpotRestApiProducts(
