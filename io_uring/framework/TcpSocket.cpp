...
int main(int argc, char *argv[]) {
    ...
    iouring::MessageHandler rwHandler = [&](iouring::Request *req, int msgSize) -> void {
        ...
        // printf("Message handler complete. - counter: %d\n", counter);    // Important: counter stays 0 or weird value
    };
    ...
    iouring::MessageHandler acceptHandler = [&](iouring::Request *req, int socketfd) -> void {
        ...
        if (clients == 1) {                             // trigger write for first client
            strncpy((char*)m_datagrams[0].msg_iov->iov_base, data, len+1);
            m_datagrams[0].msg_iov->iov_len = len+1;
            tcpSocket.Broadcast(rwHandler, &m_datagrams[0]); 
        }
