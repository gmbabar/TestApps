...
/**** L2Snapshot Msg Format
{
    ...
    :<double>}
        ...
        "symbol":<string>
        "levels":[{"price":<bouble>,"qty":<double>,"qlen":<int>,"inside":1|-1},...]
    }
}
****/
 ...
 * 
 */
inline std::string FormatL2Snapshot(const std::string& type, 
                            const int globalSeq,
                            const std::string& inside,
                            const bool isIntra,
                            const std::string& key,
                            ...
                            )
{
    ...
    aValue.SetString(levels.c_str(), static_cast<SizeType>(levels.size()), document.GetAllocator());
    document.AddMember("levels", aValue, document.GetAllocator());
...
/**
 * @brief Json L2Snapshot decoding
 * s
 */
inline void ParseL2Snapshot(const std::string& json) {
    Document document;
    ...
    std::cout << __func__ << ": levels: " << document["levels"].GetString() << std::endl;
