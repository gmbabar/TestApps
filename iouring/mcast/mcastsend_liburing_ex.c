 ...
 * gcc  -o udprecv_liburing_ex udprecv_liburing.c -luring
 * 
 */
#include <stdio.h>
#include <fcntl.h>
...
int setup_socket(int mcast_port)
{
...

	{
		...
		printf("Setting the local interface...OK\v");
	}
	...
	*/
	...
	sa.sin_addr.s_addr = inet_addr(multicast_ip);
...
// ------------------------------------------------
void setup_sendmsg_request(int sfd, struct request *req, unsigned flags)
{
	fprintf(stdout, "%s\n", __func__);
...
// ------------------------------------------------
void setup_request(struct request *req, int index)
{
	fprintf(stdout, "%s\n", __func__);
	req->event_type = EVENT_TYPE_SENDMSG;
	sprintf(req->buff, "%d: This is io_uring_sendmsg test, time: %ld", index, time(NULL));
...
{
	#define RSIZE 10
	struct request sreq[RSIZE];
	int sockfd = setup_socket(multicast_port);
	setup_context();
	for (int idx=0; idx<RSIZE; ++idx) {
		setup_request(&sreq[idx], idx);
		setup_sendmsg_request(sockfd, &sreq[idx], 0);
	}
	io_uring_submit(&ring);		// we need to do one submit once all loaded
	...
	for (int idx=0; idx<RSIZE; ++idx) {
		...
		//fprintf(stdout, "%s: Data sent: %d bytes: %s\n", __func__, cqe->res, (char*)req->msgs.msg_iov->iov_base);
	}

	fprintf(stdout, "%s: sent %d msgs\n", __func__, RSIZE);
